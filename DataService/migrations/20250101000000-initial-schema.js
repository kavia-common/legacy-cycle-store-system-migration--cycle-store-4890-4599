  ...
  async up(queryInterface, Sequelize) {
    ...
    await queryInterface.createTable('Inventory', {
      ...
      category_id: { type: Sequelize.INTEGER.UNSIGNED, allowNull: false,
        references: { model: 'Category', key: 'id' }, onUpdate: 'CASCADE', onDelete: 'RESTRICT' },
    ...
    await queryInterface.createTable('Sale', {
      id: { type: Sequelize.INTEGER.UNSIGNED, autoIncrement: true, primaryKey: true },
      customer_id: { type: Sequelize.INTEGER.UNSIGNED, allowNull: false,
        references: { model: 'Customer', key: 'id' }, onUpdate: 'CASCADE', onDelete: 'RESTRICT' },
    ...
    await queryInterface.createTable('SaleItem', {
      id: { type: Sequelize.INTEGER.UNSIGNED, autoIncrement: true, primaryKey: true },
      sale_id: { type: Sequelize.INTEGER.UNSIGNED, allowNull: false,
        references: { model: 'Sale', key: 'id' }, onUpdate: 'CASCADE', onDelete: 'CASCADE' },
      inventory_id: { type: Sequelize.INTEGER.UNSIGNED, allowNull: false,
        references: { model: 'Inventory', key: 'id' }, onUpdate: 'CASCADE', onDelete: 'RESTRICT' },
    ...
    await queryInterface.createTable('MigrationTracking', {
      ...
      entity: { type: Sequelize.STRING(100), allowNull: false },
      migrated_at: { type: Sequelize.DATE, allowNull: false, defaultValue: Sequelize.literal('CURRENT_TIMESTAMP') },
      status: { type: Sequelize.ENUM('pending', 'completed', 'failed'), allowNull: false }
    });

    // Helpful indexes
    await queryInterface.addIndex('Inventory', ['sku']);
    await queryInterface.addIndex('AuditLog', ['entity', 'entity_id']);
    await queryInterface.addIndex('SupportTicket', ['status']);
  },
